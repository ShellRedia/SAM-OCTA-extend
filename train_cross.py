...
# training
import numpy as np
from dataset import Octa500_Dataset_Cross
from options import *
...
class TrainManager_OCTA:
    def __init__(self, 
        ...
        self.fov, self.label_type, self.epochs, self.is_local, self.model_type, self.remark = parameters
...
def train_single(dataset_params, training_params, device):
    ...
    dataset_train = Octa500_Dataset_Cross(*dataset_params, True, random_seed)
    dataset_val = Octa500_Dataset_Cross(*dataset_params, False, random_seed)

    train_manager = TrainManager_OCTA(dataset_train, dataset_val, training_params, device)
    train_manager.train()

def train_profile():
    # dataset_params = [args.fov, args.label_type, args.is_local]
    # training_params = [args.fov, args.label_type, args.epochs, args.is_local, args.model_type, args.remark]
    dataset_params = [args.fov, args.label_type, False]
    training_params = [args.fov, args.label_type, args.epochs, False, args.model_type, args.remark]

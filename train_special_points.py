...
# training
import numpy as np
from dataset import Octa500_Dataset_SpecialPoints
from options import *
...
class TrainManager_OCTA:
    def __init__(self, 
        ...
        self.fov, self.label_type, self.epochs, self.is_local, self.model_type, self.point_type, self.remark = parameters
...
def train_single(dataset_params, training_params, device):
    ...
    dataset_train = Octa500_Dataset_SpecialPoints(*dataset_params, True, random_seed)
    dataset_val = Octa500_Dataset_SpecialPoints(*dataset_params, False, random_seed)

    train_manager = TrainManager_OCTA(dataset_train, dataset_val, training_params, device)
    train_manager.train()

def train_profile():
    dataset_params = [args.fov, args.label_type, args.is_local, args.point_type]
    training_params = [args.fov, args.label_type, args.epochs, args.is_local, args.model_type, args.point_type, args.remark]
